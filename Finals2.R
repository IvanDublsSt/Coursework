...
dt_averaged_for_graph <- dt[, mean(Error), by = c("Year", "Month")]
dt_averaged_for_graph[, "Year" := sapply(Year, mapyear, years = dt_averaged_for_graph$Year)]
dt_averaged_for_graph[, "Month" := sapply(Month, mapmonth, months = dt_averaged_for_graph$Month)]

png("TimeSeriesPrivate.png", width = 700, height = 400)
ggplot(dt_averaged_for_graph, aes(y=V1, x=paste(Year, Month))) + 
  geom_bar(stat = "identity") + scale_x_discrete(breaks = breaks) + theme_ipsum()+
  ylab("Error") + xlab("Date")
dev.off()
for (i in 2015:2020){
