...
write.csv(AggrTableWithRating, "BanksTableWithNormatives.csv")
View(AggrTableWithRating)
...
m <- GeneratePCTable(AggrTableWithRating[(Year>=2016)&(Month>=4)&(Status!="absent")],
                     ...
                     AggrTableWithRating[(Year>=2016)&(Month>=4)&(Status=="absent")])
n <- GenerateRestrictedPCTable(m$PCTable, 5) 

library(rlang)

ten_PC <- GenerateRestrictedPCTable(m$PCTable, 20) 
...
FindStartLength <- function(Table, method = c("logistic"), start = -0.1, to = 0.1, bycoef = 0.2, PrintErrors = F){
  ...
  return(l)}
FindStartLength(ten_PC[, -c("Month")])
...
for (i in 1:100){
  ...
  tryCatch({
    ...
    print(paste(i, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
...
RunModel <- function(Table, K = 20, CrossValidate = T, method = c("logistic"), start = -0.1, to = 0.1, bycoef = 0.2){
...
best_model <- polr(as.factor(order)~., data = ten_PC[,-c("Month")], Hess = T,method=c("logistic"), start = seq(from = -0.1, to = 0.1, by = 0.2/FindStartLength(Table)))
...
GetPrediction <- function(Table, TableNew, method = c("logistic"), start = -0.1, to = 0.1, bycoef = 0.2){
  ...
  return(list("model" = best_model, "PredictedRatings" = predicted_ratings, "FactorsAndRatings" = ))
}
