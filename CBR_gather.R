...
#У каждой из функций есть параметр save: если он равен True, то таблица сразу же сохраняется в рабочей директории в формате .csv
#и параметр year, который означает первый год в вашей выборке 
#Функция create_big_table принимает в качестве аргументов таблицы, созданные первыми двумя функциями. Параметр save работает для нее аналогичным образом.
#Инструкцию по аггрегированию столбцов см. в одноименном разделе после раздела "воспроизвести функции" 
...
replace_na <- function(x){
  ...
  return(x)}
#f2####
month_rotater <- function(month, year, beginning_year = 2015){
  nextmonth <- month+1
  tryCatch({
  if (month == 1){
    begpath <- paste("Data/101-",as.character(year+beginning_year-1),"0",as.character(month) ,"01","/12",as.character(year+beginning_year-1-1),"B1.DBF", sep = "")
    endpath <- paste("Data/101-",as.character(year+beginning_year-1),"0",as.character(nextmonth) ,"01","/0",as.character(nextmonth-1),as.character(year+beginning_year-1),"B1.DBF", sep = "")
  }
  if (month %in% seq(2,8,1)){
    begpath <- paste("Data/101-",as.character(year+beginning_year-1),"0",as.character(month) ,"01","/0", as.character(month-1),as.character(year+beginning_year-1),"B1.DBF", sep = "")
    endpath <- paste("Data/101-",as.character(year+beginning_year-1),"0",as.character(nextmonth) ,"01","/0",as.character(nextmonth-1),as.character(year+beginning_year-1),"B1.DBF", sep = "")
  }
  if (month == 9){
    begpath <- paste("Data/101-",as.character(year+beginning_year-1),"0901","/08", as.character(year+beginning_year-1),"B1.DBF", sep = "")
    endpath <- paste("Data/101-",as.character(year+beginning_year-1),"1001","/09", as.character(year+beginning_year-1),"B1.DBF", sep = "")
  }
  if (month == 10){
    begpath <- paste("Data/101-",as.character(year+beginning_year-1),"1001","/09", as.character(year+beginning_year-1),"B1.DBF", sep = "")
    endpath <- paste("Data/101-",as.character(year+beginning_year-1),"1101","/10", as.character(year+beginning_year-1),"B1.DBF", sep = "")    
  }
  if (month == 11){
    begpath <- paste("Data/101-",as.character(year+beginning_year-1),"1101","/10", as.character(year+beginning_year-1),"B1.DBF", sep = "")
    endpath <- paste("Data/101-",as.character(year+beginning_year-1),"1201","/11", as.character(year+beginning_year-1),"B1.DBF", sep = "")    
  }
  if (month == 12){
    begpath <- paste("Data/101-",as.character(year+beginning_year-1),"1201","/11", as.character(year+beginning_year-1),"B1.DBF", sep = "")
    endpath <- paste("Data/101-",as.character(year+beginning_year),"0101","/12", as.character(year+beginning_year-1),"B1.DBF", sep = "")    
  }
  
  
  print(c(begpath,  endpath))
  file_begin <- read.dbf(begpath)
  file_begin <- as.data.table(file_begin)
  file_begin[, "Month" := month]
  file_begin[, "Year" := year+beginning_year-1]
  
  file_end <- read.dbf(endpath)
  file_end <- as.data.table(file_end)
  file_end[, "Month" := month]
  file_end[, "Year" := year+beginning_year-1]  
  
  file_begin[str_detect(NUM_SC, "X"), "NUM_SC" := paste(NUM_SC, A_P, sep = "")]
  file_end[str_detect(NUM_SC, "X"), "NUM_SC" := paste(NUM_SC, A_P, sep = "")]
  
  end_table_itogo <- file_end[, c("REGN", as.character("NUM_SC"),  "Year", "IITG", "Month")]
  begin_table_itogo <- file_begin[, c("REGN", as.character("NUM_SC"),  "Year", "IITG","Month")]
  
  
  begend_table <- merge(end_table_itogo, begin_table_itogo, by = c("REGN", "NUM_SC", "Year", "Month"), all = T)
  begend_table <- begend_table[NUM_SC != "ITGAP"]
  return(begend_table)},
  error = function(cond){
    print(paste("Got error for month = ", as.character(month), ", year = ", as.character(year), sep = ""))
    print(paste("Error text:", cond, sep = ""))
    return(NULL)
  })}
#f3####
month_rotater_pnl <- function(month, year, beginning_year = 2015){
  tryCatch({
  if (month %in% c(1)){
    begpath <- paste("Data/102-",as.character(year+beginning_year-1),"0",as.character(month) ,"01","/4",as.character(year+beginning_year-1-1),"_P1.DBF", sep = "")
  }
  if (month %in% c(4)){
    begpath <- paste("Data/102-",as.character(year+beginning_year-1),"0",as.character(month) ,"01","/1", as.character(year+beginning_year-1),"_P1.DBF", sep = "")
  }
  if (month %in% c(7)){
    begpath <- paste("Data/102-",as.character(year+beginning_year-1),"0",as.character(month) ,"01","/2", as.character(year+beginning_year-1),"_P1.DBF", sep = "")
  }
  if (month %in% c(10)){
    begpath <- paste("Data/102-",as.character(year+beginning_year-1),as.character(month) ,"01","/3", as.character(year+beginning_year-1),"_P1.DBF", sep = "")}
  
  print(c(begpath))
  file_begin <- read.dbf(begpath)
  file_begin <- as.data.table(file_begin)
  file_begin[, "Month" := month]
  file_begin[, "Year" := year+2014]
  
  begin_table_itogo <- file_begin[, c("REGN", as.character("CODE"),  "Year", "SIM_ITOGO","Month")]
  begin_table_itogo[, "entries_pnl" := .N, by = REGN]
  return(begin_table_itogo)},
  error = function(cond){
    print(paste("Got error for month = ", as.character(month), ", year = ", as.character(year), sep = ""))
    print(paste("Error text:", cond, sep = ""))
    return(NULL)
...
#f4####
month_rotater_normative_supports <- function(month, year, beginning_year = 2015){
  nextmonth <- month+1
  tryCatch({
    if (month == 1){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"0",as.character(month) ,"01","/12",as.character(year+beginning_year-1-1),"_135_2.DBF", sep = "")
    }
    if (month %in% seq(2,8,1)){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"0",as.character(month) ,"01","/0", as.character(month-1),as.character(year+beginning_year-1),"_135_2.DBF", sep = "")
    }
    if (month == 9){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"0901","/08", as.character(year+beginning_year-1),"_135_2.DBF", sep = "")
    }
    if (month == 10){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"1001","/09", as.character(year+beginning_year-1),"_135_2.DBF", sep = "")
    }
    if (month == 11){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"1101","/10", as.character(year+beginning_year-1),"_135_2.DBF", sep = "")
    }
    if (month == 12){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"1201","/11", as.character(year+beginning_year-1),"_135_2.DBF", sep = "")
    }
    
    
    print(c(begpath))
    
    file_begin <- read.dbf(begpath, as.is = T)
    file_begin <- as.data.table(file_begin)
    file_begin[, "Month" := month]
    file_begin[, "Year" := year+2014]
    file_begin[, "NormativeSupport" := iconv(C1_2, from="CP866", to="UTF-8")]
    file_begin[, "Value" := iconv(C2_2, from="CP866", to="UTF-8")]
    begin_table_itogo <- file_begin[, c("REGN", "Year", "Month", "NormativeSupport", "Value")]
    return(begin_table_itogo)},
    error = function(cond){
      print(paste("Got error for month = ", as.character(month), ", year = ", as.character(year), sep = ""))
      print(paste("Error text:", cond, sep = ""))
...
month_rotater_normatives <- function(month, year, beginning_year = 2015){
  ...
  tryCatch({
    if (month == 1){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"0",as.character(month) ,"01","/12",as.character(year+beginning_year-1-1),"_135_3.DBF", sep = "")
    }
    if (month %in% seq(2,8,1)){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"0",as.character(month) ,"01","/0", as.character(month-1),as.character(year+beginning_year-1),"_135_3.DBF", sep = "")
    }
    if (month == 9){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"0901","/08", as.character(year+beginning_year-1),"_135_3.DBF", sep = "")
    }
    if (month == 10){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"1001","/09", as.character(year+beginning_year-1),"_135_3.DBF", sep = "")
    }
    if (month == 11){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"1101","/10", as.character(year+beginning_year-1),"_135_3.DBF", sep = "")
    }
    if (month == 12){
      begpath <- paste("Data/135-",as.character(year+beginning_year-1),"1201","/11", as.character(year+beginning_year-1),"_135_3.DBF", sep = "")
    }
    ...
    file_begin[, "Month" := month]
    file_begin[, "Year" := year+2014]
    file_begin[, "Normative" := iconv(C1_3, from="CP866", to="UTF-8")]
    file_begin[, "Value" := iconv(C2_3, from="CP866", to="UTF-8")]
    begin_table_itogo <- file_begin[, c("REGN", "Year", "Month", "Normative", "Value")]
    return(begin_table_itogo)},
    error = function(cond){
      print(paste("Got error for month = ", as.character(month), ", year = ", as.character(year), sep = ""))
      print(paste("Error text:", cond, sep = ""))
...
#f6####
create_bs_table <- function(beginning_year = 2015, save = F){
  begin_list <- list()
  for (i in 1:5){
    begin_list[[i]] <- do.call(rbind, lapply(seq(1,12,1),month_rotater, year = i, beginning_year = beginning_year))
  }
  
  begend_table <- do.call(rbind, begin_list)
  begend_table[, "NAsx" := ifelse(is.na(IITG.x), 1, 0)]
  begend_table[, "NAsy" := ifelse(is.na(IITG.y), 1, 0)]
  begend_table[, "NAs_begend" := NAsx + NAsy]
  begend_table[, "NAs_begend" := sum(NAs_begend), by = c("REGN", "Year", "Month")]
  begend_table[, "entries" := .N, by = c("REGN", "Year", "Month")]
  begend_table[, "NAs_begend" := NAs_begend/entries]
  begend_table[NAs_begend == 1,]$NAs_begend <- 0
  begend_table[is.na(IITG.x)]$IITG.x <- 0 
  begend_table[is.na(IITG.y)]$IITG.y <- 0 
  begend_table <- begend_table[, "IITG" := (IITG.x+IITG.y)/2][]
  begend_table[, "zeros_bs" := ifelse(is.na(IITG), 1, 0)]
  begend_table[, "zeros_number_bs" := sum(zeros_bs), by = c("REGN", "Year", "Month")]
  begend_table[, "zeros_share_bs" := zeros_number_bs/entries]
  begend_table <- begend_table[, c("REGN", "NUM_SC", "Year", "Month",  "IITG", "NAs_begend", "entries", "zeros_share_bs")]
  
  spread_table <- as.data.table(spread(begend_table, NUM_SC, IITG))
  spread_table_df <- as.data.frame(spread_table)
  spread_table_df$nas_count <- apply(spread_table_df, 1, function(x) sum(is.na(x)))
  spread_table_df[is.na(spread_table_df)] <- 0
  spread_table <- spread_table_df
  if (save == T){write.csv(spread_table, file = "bs_spread.csv")}
  return(spread_table)}
#f7####
create_normatives_table <- function(beginning_year = 2015, oldyear = 2015, oldmonth = 12, save = F){
  ...
  norm_table[, "Normative" := stri_trans_general(Normative, "russian-latin/bgn")]
  #norm_table[is.na(SIM_ITOGO), "SIM_ITOGO":=0]
  #norm_table[is.na(non_zero_entries_number_by_year), "non_zero_entries_number_by_year":= 0]
  #norm_table[,"non_zero_entries_number_by_year":= max(non_zero_entries_number_by_year), by = c("REGN", "Year", "Month")]

  
  spread_norm_table <- as.data.table(spread(norm_table, Normative, Value))
...
create_normative_supports_table <- function(beginning_year = 2015, oldyear = 2015, oldmonth = 12, save = F){
  ...
  norms_table[, "NormativeSupport" := stri_trans_general(NormativeSupport, "russian-latin/bgn")]
  #norm_table[is.na(SIM_ITOGO), "SIM_ITOGO":=0]
  #norm_table[is.na(non_zero_entries_number_by_year), "non_zero_entries_number_by_year":= 0]
  #norm_table[,"non_zero_entries_number_by_year":= max(non_zero_entries_number_by_year), by = c("REGN", "Year", "Month")]
  
  
  spread_norms_table <- as.data.table(spread(norms_table, NormativeSupport, Value))
...
for (i in 1:5){
  pnl_list[[i]] <- do.call(rbind, lapply(c(1,4, 7, 10),month_rotater_pnl, year = i, beginning_year = beginning_year))
}

pnl_table <- do.call(rbind, pnl_list)

pnl_table[, "old_format" := ifelse((as.numeric(Year) <= oldyear)&(as.numeric(Month) <= oldmonth), "pnl_old", "pnl")]
pnl_table[, "CODE" := paste(CODE, old_format, sep = "")]
pnl_table <- pnl_table[, c("REGN","CODE","Year", "SIM_ITOGO", "Month", "entries_pnl")]
oldcodes <- unique(pnl_table$CODE)[str_detect(unique(pnl_table$CODE), "old")]
newcodes <- unique(pnl_table$CODE)[str_detect(unique(pnl_table$CODE), "old") == F]
pnl_table[, "entries_number_by_year" := .N, by = c("REGN", "Year", "Month")]
pnl_table[, "NAs_pnl" := ifelse((as.numeric(Year) <= oldyear)&(as.numeric(Month) <= oldmonth), 1-(entries_number_by_year/length(oldcodes)), 1-(entries_number_by_year/length(newcodes))), by = c("REGN", "Year")]
pnl_table[SIM_ITOGO != 0, "non_zero_entries_number_by_year":= .N, by = c("REGN", "Year", "Month")]
pnl_table[is.na(SIM_ITOGO), "SIM_ITOGO":=0]
pnl_table[is.na(non_zero_entries_number_by_year), "non_zero_entries_number_by_year":= 0]
pnl_table[,"non_zero_entries_number_by_year":= max(non_zero_entries_number_by_year), by = c("REGN", "Year", "Month")]
pnl_table[, "non_zeros_pnl" := ifelse((as.numeric(Year) <= oldyear)&(as.numeric(Month) <= oldmonth), 1-(non_zero_entries_number_by_year/length(oldcodes)), 1-(non_zero_entries_number_by_year/length(newcodes))), by = c("REGN", "Year")]


spread_pnl_table <- as.data.table(spread(pnl_table, CODE, SIM_ITOGO))
spread_pnl_table_df <- as.data.frame(spread_pnl_table)
spread_pnl_table_df[is.na(spread_pnl_table_df)] <- 0
spread_pnl_table <- spread_pnl_table_df
if (save == T){write.csv(spread_pnl_table, file = "pnl_spread.csv")}
return(spread_pnl_table)
}

#f10####
create_big_table <- function(spread_pnl_table,spread_table, save = F){
  n <- merge(spread_pnl_table, spread_table, all = T, by = c("REGN", "Year", "Month"))
  cols <- colnames(n)
  n <- as.data.table(n)
  n <- as.data.table(do.call(cbind, nafill(n, "locf")))
  colnames(n) <- cols
  if (save == T){write.csv(n, file = "big_spread.csv")}
  return(n)
}
...
#Затем воспроизвести весь код отсюда и до конца файла 
#Это займет некоторое время

BigBanksTable <- big_table
...
#создание аггрегированных столбцов####
